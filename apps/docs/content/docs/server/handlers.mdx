...
Handled `UploadStuffError`s respond with status `400`, `401`, or `403` and the JSON envelope `{ "error": string }`. Unknown endpoints on known actions return `400`. Bad paths and wrong methods return `404`. In fetch and Next.js runtimes, non-`UploadStuffError` exceptions propagate to the host runtime; `toNodeHandler` forwards them to `next` when a framework provides it, otherwise it sends a `500` JSON response.
