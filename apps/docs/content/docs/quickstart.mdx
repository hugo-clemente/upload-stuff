...
Passing your server's `FileRouter` type to `createUploadStuffReactHelpers` is what
connects the two ends — every type the client hook infers flows from this one
